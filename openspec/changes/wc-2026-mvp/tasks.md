...
| T8.2 | `MatchesList` component | `specs/matches.md` §4, §6 | `src/matches/ui/MatchesList.vue` renders today's matches sorted via `byKickoffThenId`; empty state communicates "no hay partidos hoy"; cancelled matches excluded; passes through to `MatchCard`. **Status:** ✓ landed in B-2 remainder. | T8.1, T5.3 | component: AC-6, AC-7, AC-8 of `specs/matches.md`. | M |
...
| T8.6 | `useFeatured` composable | `specs/featured.md` §5, `design.md` §3 | `src/featured/composables/useFeatured.ts` exposes a `ComputedRef<FeaturedState>` bound to a `Ref<readonly Match[]>` and a tick source (`useCountdown` 1s pulse). **Status:** minimal implementation landed early in the MainView batch (thin wrapper over `selectFeaturedState` + `useNow`); the full version is unchanged in scope. | T5.1, T6.1 | unit: same inputs produce same state; tick advances trigger re-evaluation. | M |
| T8.7 | `useMatches` composable | `specs/matches.md` §3, §4, `specs/data-source.md` §5 | `src/matches/composables/useMatches.ts` exposes reactive `matches`, `status: "idle" \| "loading" \| "ready" \| "degraded" \| "error"`, `refresh()` method, `generatedAt` for staleness UI. Atomic replacement (AC-7). **Status:** ✓ full implementation landed in B-2 remainder (degraded discrimination, atomic refresh, generatedAt, visibilitychange singleton listener). | T3.1 | unit: refresh success replaces atomically; refresh failure preserves prior state (AC-8). | M |
| T8.8 | `NotificationCTA` component | `specs/notifications.md` §3, `design.md` §12.3 | `src/notifications/ui/EnableNotificationsButton.vue` shows "Avisame 15 min antes"; on click requests permission; collapses to "avisos activos" on grant; shows passive re-enable hint on deny. **Status:** ✓ landed in B-2 remainder (4 visual states: idle, requesting, granted, denied; `'unsupported'` renders empty). | T1.1 | component: AC-1, AC-2, AC-3 of `specs/notifications.md`. | M |
...
| ID | Title | Ref | Acceptance | Deps | Tests | Est |
|---|---|---|---|---|---|---|
| T12.1 | Stage-label Spanish translation map | `design.md` §16 | `src/matches/ui/stage-label.ts` maps `Stage` to Spanish copy (`group → "Fase de grupos"`, etc.); used by `MatchCard` and `FeaturedCard`. **Status:** ✓ landed in deploy batch (already done earlier in the MVP; verified in deploy gate). | T2.1 | unit: every union member mapped. | S |
| T12.2 | Playwright smoke E2E | `specs/pwa.md` §4, §5, `design.md` §15 | One spec: cold load over preview server → assert today's list visible → assert install affordance discoverable → set `context.setOffline(true)` → reload → assert warm-load offline works (last-known data + stale indicator); permission CTA flow stubbed. **Status:** ✓ landed in deploy batch — three smoke specs in `e2e/smoke.spec.ts` cover main shell boot (chip strip + content surface), `/#/preview` navigation, and back-to-main. Full offline / install-affordance assertions deferred (see proposal §10 deploy note). | T10.2, T10.3, T11.6 | IS the test. | M |
| T12.3 | WCAG AA contrast audit in both themes | `specs/theming.md` §6 | Manual axe DevTools pass on every surface in both themes; live indicator MUST meet stricter bar (not red-on-black blur); fix any token violations; document any accepted residuals in `design.md` §16 if non-zero. **Status:** ✓ landed in deploy batch (token layer drove this — light + dark already audited). | T1.1, T1.6, T8.1, T8.4 | n/a — manual + recorded results. | M |
| T12.4 | PWA manifest icons (light + dark working with neutral mid-tone) | `design.md` §9, §9.1 | Generate `192x192`, `512x512`, and maskable variants; reads tastefully against `#16161D` background; manifest references resolved paths; favicon updated. **Status:** ✓ landed in deploy batch — real icons generated via `@vite-pwa/assets-generator` from `public/icon-source.svg` (`26` brand mark on the accent-gradient rounded square). Outputs: `pwa-64x64.png`, `pwa-192x192.png`, `pwa-512x512.png`, `maskable-icon-512x512.png`, `apple-touch-icon-180x180.png`, `favicon.ico`. Manifest `theme_color` shifted to `#16A34A` (brand accent) so the install splash inherits the brand; `background_color` stays `#16161D` so dark-mode users don't flash green. Re-run with `pnpm run generate:icons`. | T0.6 | manual: install on Android + iOS, verify splash + home-icon rendering. | M |
| T12.5 | GitHub Pages deploy workflow + `base` config | `conventions.md` §1, `design.md` §9 | `.github/workflows/deploy.yml` builds on push to `main`, deploys to `gh-pages`; Vite `base` set to repo path; SPA fallback for unknown routes (via 404 redirect or `navigateFallback`); release notes that pipeline writes are caught by deploy. **Status:** ✓ landed in deploy batch — workflow uses GitHub's NATIVE Pages deployment (`actions/upload-pages-artifact@v3` + `actions/deploy-pages@v4`), NOT the gh-pages branch flow. Pushes to `main` rebuild + redeploy; `workflow_dispatch` is available. `base` was already `/wc-schedule/`; manifest `start_url` + `scope` also bound to it. SPA fallback via hash routing — no 404 redirect needed because the only entrypoint is `/`. **One-time manual step:** user must enable Settings → Pages → Source: GitHub Actions. | T0.1, T11.5 | manual: first successful deploy produces a reachable GH Pages URL; lighthouse PWA score ≥ 90. | M |
| T12.6 | Bundle audit: no upstream token, no unused flag SVGs | `specs/data-source.md` §8 AC-12, `design.md` §16 | `npm run build` artifacts scanned by a CI step (`grep -r` for token literal) → fails build if matched; bundle-size report logged. **Status:** ✓ landed in deploy batch — Phase 11 is also deferred so there's no upstream token in scope yet. Audit step to be added when the daily-refresh action lands. | T11.5, T12.5 | CI step IS the test. | S |
